...
def compute_json_metric(pred):
    ...
    with torch.no_grad():
        tokenizer = AutoTokenizer.from_pretrained(
            '/mnt/afs/niuyazhe/model', add_eos_token=False, trust_remote_code=True, use_fast=False)
        ...
        # 保存到pickle文件
        result_file = f"/mnt/afs/xueyingyi/meme/storage/result/result_{unique_id}.pickle"
        label_file = f"/mnt/afs/xueyingyi/meme/storage/label/label_{unique_id}.pickle"
...
def load_pickle_files(folder_path):
    ...
    for file_name in pickle_files:
        ...
        with open(file_path, 'rb') as f:
            data.append(pickle.load(f))  # 读取pickle文件的内容并添加到列表中
    return data


def compute_json_metric_eval(pred):
    ...
    sentiment_dict = {key: 0 for key in sentiment_category}
    intention_dict = {key: 0 for key in intention_detection}
    sentiment_length_dict = {key: 0 for key in sentiment_category}
    intention_length_dict = {key: 0 for key in intention_detection}
    ...
    acc_dict = {key: 0 for key in key_list}

    folder_path = "/mnt/afs/xueyingyi/meme/storage"  # pickle文件存放目录
    result_data = load_pickle_files(folder_path + "/result")  # result文件夹中存放了所有result的pickle文件
    label_data = load_pickle_files(folder_path + "/label")  # label文件夹中存放了所有label的pickle文件

    # 读取结果和标签并逐个处理
    count = 0
    for r, l in zip(result_data, label_data):
        try:
            r_dict = json.loads(r)
            ...
            print(f"Error decoding JSON for count {count}: {e}")
            continue
