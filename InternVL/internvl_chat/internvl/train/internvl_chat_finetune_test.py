...
def compute_json_metric(pred):
    ...
    with torch.no_grad():
        ...
        for id, pred in enumerate(labels):
            # 查找第一个值为 -100 的位置
            result_null = np.where(result[id] == -100)[0]
            # 查找第一个值不为 -100 的位置
            result_start = np.where(result[id] != -100)[0]
            # 查找第一个值为 -100 的位置
            label_null = np.where(labels[id] == -100)[0]
            # 查找第一个值不为 -100 的位置
            label_start = np.where(labels[id] != -100)[0]

            # 将所有这些位置的值替换为 2
            result[id][result_null] = 2
            result[id][label_null] = 2

            # 类似地：
            labels[id][label_null] = 2
        ...
        for r, l in zip(result, label):
            ...
            try:
                ...
                for key in key_list:
                    ...
                    print('////////////////////result_dict_sim///////////////////')
                    print(count,r_dict)
                    print('////////////////////label_dict///////////////////')
                    print(count,label_dict)
                    ...
                    if r_dict[key] == label_dict[key]:
                        ...
                        if key == 'sentiment_category':
                            ...
                            wrong_list.append([count, r_dict['intention_detection'], label_dict['intention_detection'], label_dict])
            count += 1

        # with open('/mnt/afs/niuyazhe/data/meme/InternVL/result_check.csv', 'w') as csvfile:
