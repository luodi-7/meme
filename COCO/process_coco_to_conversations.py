...
# 输入文件路径
input_file = '/mnt/afs/xueyingyi/COCO/train_process/processed_coco_annotations.jsonl'
output_file = '/mnt/afs/xueyingyi/COCO/train_process/train_data.jsonl'
...
for line in lines:
    ...
    image_path = f"/mnt/afs/xueyingyi/COCO/train2017/{file_name}"
    ...
    for category_name, bboxes in category_annotations.items():
        ...
        # 构建"human"提问
        human_value = f"Recognize the position of {category_name} in the picture"
        
        # 将所有的bbox合并为一个列表的形式，逗号分隔
        gpt_value = f"[{', '.join(bboxes)}]"
