...
def bbox_to_loc(bbox, image_width, image_height):
    ...
    # 格式化为 <loc> 标签
    loc_str = f"<loc{y_min_norm:04}><loc{x_min_norm:04}><loc{y_max_norm:04}><loc{x_max_norm:04}> text location"
    return loc_str

# 输入文件和输出文件路径
input_jsonl_path = "/mnt/afs/xueyingyi/image_vague/loc_dection/dections_shaoang.jsonl"
...
with open(input_jsonl_path, "r") as input_file, \
    ...
    for line in input_file:
        ...
        # 转换 detections 中的 bbox 为 <loc> 标签格式
        transformed_detections = []
        for detection in data["detections"]:
            bbox = detection["bbox"]
            loc_str = bbox_to_loc(bbox, image_width, image_height)
            transformed_detections.append(loc_str)
